...
The existing version-1 transport also hosts `meta/muse-image`: exact raw prompt, ordered PNG/JPEG/WebP references, saved size and n=1, with no Qwen seed. A Muse receipt retains native bytes, true media type, normalized raster hash and actual/unknown cost. `recover` reconstructs pixels from that exact persisted receipt without a provider request. Qwen is retained for historical tests but its real client refuses execution. `inheritedQwenPythonAdapter(true)` permits loading a recorded result without a key; a new dispatch still requires its logical credential.
