...
| python-api.qwen_ui_pipeline.providers.alibaba.AlibabaImageClient.generate | retained implementation | Generation.invoke through OpenRouter | Issue #29 migrates remaining ComfyUI callers; Issue #30 removes direct Alibaba reachability. |
| python-api.qwen_ui_pipeline.providers.alibaba.build_alibaba_request | retained implementation | Generation.prepare through the OpenRouter adapter protocol | Issue #30 retains captured request-read fixtures before the direct builder is retired. |
| python-api.qwen_ui_pipeline.providers.openrouter.OpenRouterImageClient.generate | retained implementation | Generation.invoke | Issue #28 places the client behind the versioned Qwen kernel; Issues #29 and #30 remove the remaining recorded direct callers and public reachability. |
...
| python-api.qwen_ui_pipeline.qwen_adapter.invoke_qwen_kernel | retained implementation | Generation.inheritedQwenAdapter through adapter protocol version 1 | Issue #30 may contract the kernel only after the versioned adapter and cross-process qualification replace it. |
...
| python-cli.generate | compatibility adapter | Conductor.plan and Conductor.advance | Issue #30 may remove the deprecated command after saved-input callers adopt Conductor plan and advance. |
