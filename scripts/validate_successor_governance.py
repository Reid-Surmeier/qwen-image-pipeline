...
def validate_repository(root: Path) -> list[str]:
    ...
    if not verify.is_file():
        ...
        if "resolve_bootstrap_python()" not in verify_text:
            problems.append("canonical baseline does not resolve its bootstrap interpreter from trusted paths")
        if "/usr/bin/python3.12" not in verify_text:
            problems.append("canonical baseline is missing the system Python bootstrap path")
        if "/usr/local/bin/python3.12" not in verify_text:
            problems.append("canonical baseline is missing the local Python bootstrap path")
