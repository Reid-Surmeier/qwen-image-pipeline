...
| Generation | Prove locked references reach exact provider payload locations, authorize one submission, and normalize Qwen or Seedance adapter evidence. | `modules/generation/index.ts` | `modules/generation/errors.ts` | `modules/generation/generation.test.ts`, `modules/generation/inherited-qwen-adapter.test.ts`, `modules/generation/python-qwen-kernel-transport.test.ts`, `tests/test_qwen_adapter.py`, and `tests/test_qwen_adapter_host.py` |
