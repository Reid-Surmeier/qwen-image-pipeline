...
| Generation | Prove locked references reach exact provider payload locations, authorize one submission, and normalize Qwen or Seedance adapter evidence. | `modules/generation/index.ts` | `modules/generation/errors.ts` | `modules/generation/generation.test.ts` and `modules/generation/inherited-qwen-adapter.test.ts` |
