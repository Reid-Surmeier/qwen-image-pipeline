...
The installed tool artifact contains a closed `tool-artifact.json` inventory plus `RELEASE`, `COMMIT`, and `VERSION_PROFILE.json`. Planning verifies the complete inventory and hashes before deriving identity. The Tool Lock must exactly match that derived release, commit, aggregate artifact SHA-256, Procedure version, Run schema version, and adapter protocol version. Advancement re-reads the current Project Contract and Tool Lock, reverifies the installed artifact bytes, and checks the canonical Planned Run against the selected Procedure's exact model, mode, OpenRouter provider, version, count/cost ceilings, and reference requirements before it can reserve an attempt. A caller-provided identity, forged request, partial match, floating match, post-plan drift, unlisted file, or changed artifact is refused.
